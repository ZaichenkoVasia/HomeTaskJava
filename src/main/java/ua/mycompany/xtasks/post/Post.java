...
public class Post {
    ...
    public Letter[] takeLettersByReceiver(Person person) {
    ...
    public Letter[] takeLettersBySender(Person person) {
        Letter[] personLetters = new Letter[10];
        int personCount = 0;
        for (int i = 0; i < letterCount; i++) {
    ...
    public Letter[] takeLettersBySenderAndReceiver(Person sender, Person receiver) {
        Letter[] personLetters = new Letter[10];
        int personCount = 0;
        for (int i = 0; i < letterCount; i++) {
            if (sender == letters[i].getSender() && receiver == letters[i].getReceiver()) {
                if (personCount == personLetters.length) {
                    personLetters = Arrays.copyOf(personLetters, personLetters.length + 5);
                }
                personLetters[personCount++] = letters[i];
